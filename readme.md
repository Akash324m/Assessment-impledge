...
My logic uses a recursive approch along with memoization, where the recursive function is used to break down a word into potential prefix and sufix, where as memoization is there to optimize the recursive process by preventing the program from re-calculating the same word segments multiple times, reducing the execution time. 
